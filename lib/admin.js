...
async function discover(req, res) {
	...
	try {
		...
		const metadata = await discovery.discover(issuer, {
			allowLoopbackProviderEndpointsForDevelopment: settings.allowLoopbackProviderEndpointsForDevelopment,
		});
...
async function testJwks(req, res) {
	...
	try {
		...
		logger.warn('jwks test failed', { jwksUri, code: err.code, message });
		if (err.statusCode || err.errors) {
			return res.status(err.statusCode || 400).json({ message, errors: err.errors || {} });
		}
		if (err.code) {
...
async function getLastAuthorizationStart(req, res) {
	res.json(await diagnostics.getLastAuthorizationStart() || {});
}
