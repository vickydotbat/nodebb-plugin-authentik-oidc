...
async function acquireDbLinkLock(uid) {
	...
	const count = await db.incrObjectField(LINK_LOCK_KEY, field);
	if (count > 1) {
...
async function linkSubUnlocked(uid, claims, issuer) {
	...
	const writes = [
		...
		db.setObjectField(subKey(claims.sub), 'uid', uid),
...
async function resolve(claims, options = {}) {
	...
	if (!normalized.emailVerified) {
		logger.warn('unverified email', { sub: normalized.sub, email: normalized.email });
